...
# ── Colores ───────────────────────────────────────────────────────
BG="#0A1628"; DARK="#0D2137"; MID="#1A3A5C"; PRI="#1E5FAD"
BRT="#2E86DE"; ACC="#54A0FF"; PALE="#A8D8F0"; WHITE="#EEF6FF"
GOLD="#FFC300"; GREEN="#00B894"; YEL="#FDCB6E"; RED="#E17055"; TEXT="#DDE8F5"
...
def TH(**kw):
    d = dict(
        ...
        plot_bgcolor="rgba(13,33,55,0.7)",
        font_color=TEXT,
        font_family="DM Sans",
...
def rbadge(r):
    m = {"muy_alta":("Muy Alta","a"),"alta":("Alta","a"),"media_alta":("Media-Alta","m"),
         "media":("Media","m"),"baja":("Baja","b")}
    l, c = m.get(r, ("N/A","m"))
    return f'<span class="{"ba" if c=="a" else "bm" if c=="m" else "bb"}">{l}</span>'

def ticon(t):
    return {"muy_creciente":"🚀 Muy Creciente","creciente":"📈 Creciente",
            "estable":"➡️ Estable","decreciente":"📉 Decreciente"}.get(t, t)
...
PM = {
    "Maquillaje":["Sephora","Ulta","Amazon"],"Piel":["Amazon","Dermstore","Ulta"],
...
def _orig(b):
    ...
    for k,v in {"loreal":"Francia","lancome":"Francia","charlotte tilbury":"UK",
                ...
                "cerave":"USA","benefit":"USA","elf":"USA","nivea":"Alemania",
                "some by mi":"Korea","shiseido":"Japon"}.items():
        if k in bl: return v
    return "USA"
        ...
        for (pt, cat, sub, tgt, prob) in MSEARCHES:
            try:
                ...
                items = [p for p in r.json()
                         if p.get("name") and float(p.get("price") or 0) > 0][:5]
                for p in items:
                    ...
                    out.append({
                        "id": f"API{len(out)+1:04d}", "cat": cat, "sub": sub,
                        "nombre": (p.get("name") or "")[:80], "marca": brand,
                        "origen": _orig(brand), "precio": price,
                        "mercado": round(price*1.12, 2), "unidades": revs,
                        "rating": round(rat,1), "reviews": revs,
                        "tend": _tend(rat, revs), "rot": _rot(revs),
                        "desc": (p.get("description") or p.get("name") or "")[:100],
                        "target": tgt, "problema": prob,
                        "plat": PM.get(cat, ["Amazon","Sephora"]),
...
def build_df(api=[]):
    rows = [{**p, "fuente":"Base Local"} for p in LP] + list(api)
...
# ══════════════════════════════════════════════════════════════════
# PASO 2: SIDEBAR — filtros (después de que DF existe)
# ══════════════════════════════════════════════════════════════════
with st.sidebar:
    st.title("🎛️ Filtros")
    st.divider()
    ...
    ts    = st.multiselect("📈 Tendencia",
                ...
                format_func=lambda x: {"muy_creciente":"🚀 Muy Creciente",
                    "creciente":"📈 Creciente","estable":"➡️ Estable",
                    "decreciente":"📉 Decreciente"}[x])
    ...
    rm    = st.slider("⭐ Rating mín.", 1.0, 5.0, 4.0, 0.1)
    st.divider()
    ...
    ac = len(api_rows)
    if ac > 0:
        st.success(f"🟢 API activa — {ac} productos en vivo")
    else:
        st.info("⚡ Datos locales — " + str(len(LP)) + " productos")
    st.caption(f"Total: {len(DF)} productos")
...
df = df[
    (df["precio"] >= pr[0]) & (df["precio"] <= pr[1]) &
    df["tend"].isin(ts) & df["rot"].isin(rs) &
    (df["rating"] >= rm)
]
...
with t1:
    ...
    if len(top) == 0:
        ...
        bar_colors = [ACC if i==0 else BRT if i==1 else PRI if i<4 else MID for i in range(len(top))]
        ...
        lbl = {"unidades":"Unidades/mes","vol_usd":"Volumen USD","score":"Score","rating":"Rating"}
        fig.update_layout(**TH(height=400, showlegend=False,
            margin=dict(l=10,r=100,t=30,b=10),
            yaxis_categoryorder="total ascending",
            title_text=f"Top 10 por {lbl.get(sb,sb)}"))
        ...
        for i, row in top.iterrows():
            ...
            with st.expander(f"{md} {row['nombre']} — {row['marca']} · ${row['precio']:.2f}", expanded=(rk<=2)):
                ...
                <tr><td style="padding:4px 10px;color:{PALE};font-size:.82rem;">Origen</td><td style="padding:4px 10px;color:{WHITE};">{row["origen"]}</td>
                    <td style="padding:4px 10px;color:{PALE};font-size:.82rem;">Categoria</td><td style="padding:4px 10px;color:{WHITE};">{row["cat"]} › {row["sub"]}</td></tr>
...
with t2:
    ...
    with c1:
        ...
        tc.columns = ["tend","n"]
        tc["label"] = tc["tend"].map({"muy_creciente":"Muy Creciente","creciente":"Creciente",
                                       "estable":"Estable","decreciente":"Decreciente"})
    ...
    with c2:
        ...
        vc_colors = [ACC if i>=len(vc)-2 else BRT if i>=len(vc)-4 else PRI if i>=len(vc)-7 else MID for i in range(len(vc))]
        fig = go.Figure(go.Bar(
            y=vc.index, x=vc.values, orientation="h",
            marker_color=vc_colors,
            text=[fmt(v) for v in vc.values],
            textposition="outside", textfont=dict(color=WHITE)))
        fig.update_layout(**TH(title_text="Volumen Mensual por Categoria (USD)", height=300,
                               showlegend=False, margin=dict(l=10,r=90,t=50,b=10)))
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig.add_trace(go.Scatter(x=ml, y=pl, name="Precio USD",
        line_color=GOLD, line_width=3, mode="lines+markers"), secondary_y=False)
    fig.add_trace(go.Bar(x=ml, y=ul, name="Unidades",
        marker_color=ACC, opacity=0.7), secondary_y=True)
    fig.update_layout(**TH(title_text=f"Precio vs Rotacion — {sel[:45]}",
    ...
    insight = ("Alerta: Precio baja sin que la rotacion suba." if pchg<-3 and uchg<5
               else "Saludable: Rotacion creciente independiente del precio." if uchg>0
               else "Comportamiento normal de mercado.")
    st.markdown(f"""<div class="card">
    <div style="font-family:Syne,sans-serif;font-weight:700;color:{WHITE};margin-bottom:10px;">Insight Automatico</div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;">
      <div><span style="color:{PALE};font-size:.78rem;">Variacion precio 12m</span><br>
           <span style="color:{GOLD};font-size:1.2rem;font-weight:700;">{"+" if pchg>0 else ""}{pchg:.1f}%</span></div>
      <div><span style="color:{PALE};font-size:.78rem;">Variacion rotacion 12m</span><br>
           <span style="color:{ACC};font-size:1.2rem;font-weight:700;">{"+" if uchg>0 else ""}{uchg:.1f}%</span></div>
      <div><span style="color:{PALE};font-size:.78rem;">Correlacion Precio-Rotacion</span><br>
           <span style="color:{GREEN};font-size:1.2rem;font-weight:700;">{corr:.2f}</span></div>
    </div>
    <div style="margin-top:10px;padding-top:8px;border-top:1px solid {PRI}30;color:{TEXT};font-size:.84rem;">{insight}</div>
    </div>""", unsafe_allow_html=True)

# ─── TAB 3: PRECIOS & ROTACION ────────────────────────────────────
with t3:
    c1, c2 = st.columns(2)
    with c1:
        fig = px.scatter(DF, x="precio", y="rating", size="unidades", color="cat",
            hover_name="nombre", color_discrete_sequence=px.colors.sequential.Blues_r,
    ...
    with c2:
        ...
        fig.update_layout(**TH(title_text="Volumen por Rango de Precio",
    ...
    fig.update_layout(**TH(title_text="Unidades/mes — Categoria x Tendencia",
...
with t4:
    search = st.text_input("🔎 Buscar por nombre, marca o problema...",
                           placeholder="Ej: acne, retinol, caida cabello...")
    dfe = DF.copy()
    if search:
        q = search.lower()
        dfe = dfe[
            dfe["nombre"].str.lower().str.contains(q,na=False) |
    ...
    st.caption(f"Mostrando {len(dfe)} productos")
    if len(dfe) == 0:
        st.warning("Sin resultados.")
    else:
        disp = dfe[["id","nombre","marca","cat","precio","mercado","margen",
                    "unidades","rating","tend","rot","target","fuente"]].copy()
        disp.columns = ["ID","Producto","Marca","Categoria","Precio","Mercado",
                        "Margen%","Unid/mes","Rating","Tendencia","Rotacion","Target","Fuente"]
        disp["Tendencia"] = disp["Tendencia"].map({"muy_creciente":"🚀 Muy Creciente",
            "creciente":"📈 Creciente","estable":"➡️ Estable","decreciente":"📉 Decreciente"})
        disp["Rotacion"] = disp["Rotacion"].map({"muy_alta":"Muy Alta","alta":"Alta",
            "media_alta":"Media-Alta","media":"Media","baja":"Baja"})
        ...
        csv = dfe.drop(columns=["plat"], errors="ignore").to_csv(index=False).encode("utf-8")
...
with t5:
    ...
    with c1:
        ...
        fig.update_layout(**TH(title_text="Mapa de Oportunidades por Volumen USD",
    ...
    with c2:
        ...
        fig.update_layout(**TH(title_text="Top 10 Mayor Margen vs Mercado",
            barmode="group", height=400, margin=dict(l=10,r=10,t=50,b=90),
            xaxis_tickangle=-35,
            legend=dict(orientation="h",y=1.05,bgcolor="rgba(0,0,0,0)")))
        st.plotly_chart(fig, use_container_width=True)
    ...
    rc = DF.groupby("cat").agg(
        rating=("rating","mean"), margen=("margen","mean"),
        unidades=("unidades", lambda x: x.mean()/1000),
        score=("score","mean"),
        precio=("precio", lambda x: max(0.0, 10-(x.mean()/30)))
    ).reset_index()
    cats6  = rc["cat"].head(6).tolist()
    mlab   = ["Rating","Margen","Vol(K)","Score","Precio Comp."]
    mcol   = ["rating","margen","unidades","score","precio"]
    mmax   = [5, 15, 120, 120, 10]
    cols6  = [ACC, GOLD, GREEN, YEL, PALE, BRT]
    fig = go.Figure()
    for i, cat in enumerate(cats6):
        rrow = rc[rc["cat"]==cat].iloc[0]
        vals = [min(float(rrow[m])/(mx or 1)*10, 10) for m,mx in zip(mcol,mmax)]
        vals.append(vals[0])
        fig.add_trace(go.Scatterpolar(
            r=vals, theta=mlab+[mlab[0]], fill="toself", name=cat,
            opacity=0.8,
            line_color=cols6[i % len(cols6)], line_width=2))
